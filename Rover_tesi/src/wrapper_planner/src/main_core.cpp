...
void AllineaRobot(int gradi)
{
	...
	//prendo il tf del base_footprint 
...
void Action(nav_msgs::OccupancyGrid mappe)
{
   ...
   std::vector<int> celleOccupate= CelleOccupate(mappaStorica,sizemap,height);
	...
	int x,y;
	//aggiungiamo gli ostacoli. L'alternariva sarebbe farla con la set_obstacles
	...
	p1.compute_GPU_path(2,true,path);
